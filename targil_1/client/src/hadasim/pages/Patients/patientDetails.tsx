      ...
      {patient && (
        <DialogContent>
         ...
         {patient.birthDate&&
          ...
          {selectedCorona1?.date__of_recovery && <Typography>Date of recovery {format(selectedCorona1?.date_receiving_positive_result, 'dd/MM/yyyy')}</Typography>}
          {selectedCorona1?.Date_vaccination && <Typography> num vaccinations: {selectedCorona1.Date_vaccination.length}</Typography>}

          {selectedCorona1?.Date_vaccination.map((v, index) =>
